...
std::vector<T> CountingSort<T>::sort() {
    ...
    if (m < 0) {
        transform(vec.begin(), vec.end(), vec.begin(), bind2nd(std::plus<int>(), -1 * m));
        range += m;
    }
    ...
    //shift vector back
    if (m < 0) {
        transform(Sort<T>::sorted_data.begin(), Sort<T>::sorted_data.end(), Sort<T>::sorted_data.begin(), bind2nd(std::plus<int>(), m));
    }
